...
void addHistory(char* userInput) {
    ...
    oldest = (oldest+1) % (HISTLEN+1);
    h_index = (h_index+1) % (HISTLEN+1);
}
...
void printHistory() {
    ...
    for (int i=oldest + 1 ; i != oldest ; i = i+1 % HISTLEN) {
...
void execute (cmdLine *pCmdLine, process** process_list){
    ...
    if (pCmdLine->next != NULL) {
        ...
        printf("here1");
        cmdLine* old = parseCmdLines(cHistory[oldest]);
        printf("here2");
        addHistory(cHistory[oldest]);  //HERE!!
        printf("here3");
        execvp(old->arguments[0],old->arguments);
        perror("execv failed!");
        freeCmdLines(pCmdLine); //if succeeded wont get here and then how will free the cmdLine???
    }
    /* add "=" to the atoi >= 0*/
    else if((pCmdLine->arguments[0][0])== '!' && atoi(pCmdLine->arguments[0]+1)-1 >= 0 && atoi(pCmdLine->arguments[0]+1)-1 <= HISTLEN) {
    //else if(strcmp(strncpy(pCmdLine->arguments[0],0,1), "!") == 0 && atoi((pCmdLine->arguments[0])[1])-1 > 0 && atoi((pCmdLine->arguments[0])[1])-1 <= HISTLEN) {
        printf("here5");
        cmdLine* old = parseCmdLines(cHistory[atoi(pCmdLine->arguments[0]+1)-1]);
        addHistory(cHistory[atoi((pCmdLine->arguments[0])+1)-1]);
        execvp(old->arguments[0],old->arguments);
        perror("execv failed!");
        freeCmdLines(pCmdLine);;
    }
